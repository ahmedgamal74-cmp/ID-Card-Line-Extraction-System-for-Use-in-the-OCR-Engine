...
# correlation on a fixed area with respect to a set of reference images to enhance/generalize correlation
...
"""
Normalized cross correlation on the green channel
...
def corr_green(roi_ref, roi_in):
    a = roi_ref[..., 1]     # working on the 3 ch gives great results too 
    b = roi_in[..., 1]
    a = a.astype(np.float32).ravel() # flatten
    b = b.astype(np.float32).ravel()
    a -= a.mean(); b -= b.mean()    # zero center 
    return float(((a * b) / (a.std() * b.std() + 1e-6)).mean())
...
def main():
    ...
    ref_h, ref_w = ref_imgs[0][1].shape[:2] # 631, 1000 

    # calculate averaged reference ROI
    ref_roi_mean = average_ref_roi(ref_imgs, corr_roi)
    ...
    for img_path in img_paths:
        ...
        score = corr_green(ref_roi_mean, in_roi)
        # check if needs derotation or not 
        if score >= corr_thresh:
            out_path = save_dir / img_path.name
            cv2.imwrite(str(out_path), img)
            correct += 1
            print(f"Image {img_path.name} with score = {score:.4f} correct and saved to {out_path.name}")
        else:
            ...
            derotated += 1
            print(f"Image {img_path.name} with score = {score:.4f} derotated and saved to {out_path.name}")
        total+=1    
